...
function LinkControl({
  ...
  const [isEditingLink, setIsEditingLink] = useState(
    forceIsEditingLink !== undefined ? forceIsEditingLink : !value || !value.url
  )
  ...
  const handleOnCreate = async (suggestionTitle) => {
    ...
    try {
      ...
      setErrorMessage(
        error.message ||
          __('An unknown error occurred during creation. Please try again.')
      )
      setIsResolvingLink(false)
      setIsEditingLink(true)
    }
  ...
  const renderSearchResults = ({
    ...
    const shouldShowCreateSuggestion =
      createSuggestion && !isSingleDirectEntryResult && !isInitialSuggestions
            ...
            if (shouldShowCreateSuggestion && CREATE_TYPE === suggestion.type) {
              return (
                <LinkControlSearchCreate
                  ...
                  onClick={async () => {
                    await handleOnCreate(suggestion.title)
                  }}
      ...
      {(isEditingLink || !value) && !isResolvingLink && (
        <LinkControlSearchInput
          ...
          onSelect={async (suggestion) => {
            ...
            } else if (!noDirectEntry || Object.keys(suggestion).length > 1) {
